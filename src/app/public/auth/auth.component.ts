...
export class AuthComponent {
  ...
  form = { email: 'admin@livwell.ae', password: 'Admin@123', remember: false };
  ...
  forgotEmail = signal('');
  forgotOtp   = signal('');
  // Demo OTP — in prod this comes from backend
  ...
  submitForgotEmail(): void {
    ...
    setTimeout(() => {
      ...
      this.loading.set(false);
      if (!exists) {
        this.forgotError.set('No account found with that email address.');
        return;
      }
  ...
  private _startCountdown(seconds: number): void {
    ...
    this._countdownTimer = setInterval(() => {
      ...
      if (current <= 1) {
        ...
        this.otpResendCountdown.set(current - 1);
      }
    }, 1000);
  }
