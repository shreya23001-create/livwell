...
const STATIC_AVATARS: Record<string, string> = {
  Buying:     'https://randomuser.me/api/portraits/women/44.jpg',
  Investment: 'https://randomuser.me/api/portraits/men/32.jpg',
  Legal:      'https://randomuser.me/api/portraits/women/68.jpg',
  Finance:    'https://randomuser.me/api/portraits/women/52.jpg',
  Market:     'https://randomuser.me/api/portraits/men/45.jpg',
  Renting:    'https://randomuser.me/api/portraits/men/67.jpg',
  General:    'https://randomuser.me/api/portraits/men/32.jpg',
};

const STATIC_AUTHORS: Record<string, string> = {
  Buying:     'Sarah Al Mansouri',
  Investment: 'Khalid Rehman',
  Legal:      'Priya Nair',
  Finance:    'Fatima Al Zahra',
  Market:     'Ahmed Hassan',
  Renting:    'Omar Farsi',
  General:    'Livwell Team',
};
...
export class FaqComponent implements OnInit {
  ...
  allFaqs = signal<Faq[]>(FALLBACK_FAQS);
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (!error && data && (data as DbFaq[]).length > 0) {
      this.allFaqs.set((data as DbFaq[]).map(f => ({
        id:       f.id,
        question: f.question,
        answer:   f.answer,
        category: f.category,
        author:   STATIC_AUTHORS[f.category] ?? 'Livwell Team',
        avatar:   STATIC_AVATARS[f.category] ?? 'https://randomuser.me/api/portraits/men/32.jpg',
        date:     '',
      })));
    }
  }
  ...
  filteredFaqs = computed(() => {
    ...
    return cat === 'All' ? list : list.filter(f => f.category === cat);
  });
